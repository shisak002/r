...
# ---------------- CONFIG ----------------
BOT_TOKEN = "8500713256:AAHBscSwbKwtJ6gvf5r1tJJLEnsjKcxxOIk"
...
# ---------- START / STOP ----------
def start_watcher(chat_id, base_url, silent=False):
...
# ---------- AUTO START DEFAULT FIREBASE ----------
def auto_start_default_firebase():
    """Automatically start default Firebase monitoring for all owners when bot starts"""
    if not AUTO_START_DEFAULT_FIREBASE:
        return
    
    if not DEFAULT_FIREBASE_URL or DEFAULT_FIREBASE_URL.strip() == "":
        print("❌ DEFAULT_FIREBASE_URL is empty. Skipping auto-start.")
        return
    
    test_url = normalize_json_url(DEFAULT_FIREBASE_URL)
    if not http_get_json(test_url):
        print(f"❌ Default Firebase URL is not accessible: {DEFAULT_FIREBASE_URL}")
        return
    
    # Start monitoring for all owners
    for owner_id in OWNER_IDS:
        try:
            start_watcher(owner_id, DEFAULT_FIREBASE_URL, silent=True)
            print(f"✅ Auto-started default Firebase monitoring for owner: {owner_id}")
            send_msg(
                owner_id,
                f"🤖 <b>Bot Started Automatically</b>\n\n"
                f"✅ Default Firebase monitoring started:\n"
                f"<code>{html.escape(DEFAULT_FIREBASE_URL)}</code>\n\n"
                f"Use /stop to stop monitoring."
            )
        except Exception as e:
            print(f"❌ Failed to auto-start monitoring for owner {owner_id}: {e}")
...
def handle_update(u):
    ...
    if lower_text == "/start":
        ...
        # Check if admin already has default Firebase running
        if is_owner(chat_id):
            if firebase_urls.get(chat_id) == DEFAULT_FIREBASE_URL:
                admin_commands += f"\n✅ <b>Default Firebase is active</b>\n<code>{html.escape(DEFAULT_FIREBASE_URL)}</code>"
    ...
    if lower_text == "/ping":
        ...
        # Check default Firebase status
        default_active = 0
        for uid, url in firebase_urls.items():
            if url == DEFAULT_FIREBASE_URL:
                default_active += 1
        ...
        if DEFAULT_FIREBASE_URL and is_owner(chat_id):
            status_text += f"\n🔗 Default Firebase active for: <code>{default_active}</code> admin(s)"
    ...
    # /stop
    if lower_text == "/stop":
        # Check if this is the default Firebase
        current_url = firebase_urls.get(chat_id)
        if current_url == DEFAULT_FIREBASE_URL and is_owner(chat_id):
            # Ask confirmation for stopping default Firebase
            reply_markup = {
                "inline_keyboard": [
                    [
                        {"text": "✅ Yes, stop it", "callback_data": f"confirm_stop_{chat_id}"},
            ...
            send_msg(
                chat_id,
                "⚠️ <b>Warning!</b>\n\n"
                "You are about to stop the <b>DEFAULT FIREBASE</b> monitoring.\n"
                "This is automatically started when bot restarts.\n\n"
    ...
    if lower_text == "/list":
        ...
        if is_owner(chat_id):
            if not firebase_urls:
                ...
                # Check if using default Firebase
                default_tag = ""
                if user_url == DEFAULT_FIREBASE_URL:
                    default_tag = "\n✅ <b>Using DEFAULT Firebase</b>"
    ...
    if lower_text == "/adminlist":
        ...
        for uid, url in firebase_urls.items():
            # Check if this is the default Firebase
            default_tag = " (DEFAULT)" if url == DEFAULT_FIREBASE_URL else ""
            if url == DEFAULT_FIREBASE_URL:
                default_count += 1
            lines.append(
                f"👤 <code>{uid}</code> -> <code>{html.escape(str(url))}</code>{default_tag}"
            )
        
        header = f"👑 <b>All active Firebase URLs (admin only)</b>\n"
        if DEFAULT_FIREBASE_URL:
            header += f"Default Firebase active for: <code>{default_count}</code> admin(s)\n\n"
        
        send_msg(chat_id, header + "\n".join(lines))
        return
...
def handle_callback_query(update):
    ...
    # Only owners can interact with these buttons
        ...
        try:
            ...
            if target_chat_id == chat_id:
                ...
                requests.post(f"{API_URL}/editMessageText", json={
                    ...
                    "text": "✅ Default Firebase monitoring stopped.",
                    "parse_mode": "HTML"
                })
        except:
            pass
...
# ---------- MAIN LOOP ----------
def main_loop():
    # Auto-start default Firebase for all owners
    auto_start_default_firebase()
